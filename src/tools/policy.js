...
export function registerPolicy(server) {
  registerEnabledTool(server, "bridge_instructions", {
    ...
    "Create a new isolated Git branch/worktree with workspace_create. Use workspace_attach for an existing directory in place or an exact existing branch at an expected head; it never creates or replaces a branch. A remote-only branch uses a detached exact-head worktree. Git-only tools apply only when the selected directory is the Git top level.",
    "For an existing branch attached with an explicit remote, git_publish_attached_branch publishes only the current fast-forward HEAD to that recorded remote and branch after expected-head verification, then requires exact readback. It accepts no caller-selected target or force mode and never creates a PR.",
    "Mutations are serialized per real workspace path. This profile is non-exclusive: it does not claim universal coordinator interception, and coordinator-required operations still honor BLOCK, REFRESH, and UNAVAILABLE.",
